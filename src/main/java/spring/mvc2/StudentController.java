...
public class StudentController {
    ...
    public @ResponseBody String getAllStudent(@ModelAttribute("formBean") FormBean formBean, Model model) {
        ...
        Query query = session.createQuery(" from Student"); // select name, id,
                                                            // age from Student
                                                            // //from Student
    ...
    @RequestMapping(value = "get/all2", method = RequestMethod.GET)
    public @ResponseBody String getAllStudent2(@ModelAttribute("formBean") FormBean formBean, Model model) {
    ...
    @RequestMapping(value = "updateFromTable", method = RequestMethod.POST, consumes = "application/json", produces = "application/json")
    ...
    @RequestMapping(value = "deleteFromTable", method = RequestMethod.POST, consumes = "application/json", produces = "application/json")
    // produces = MediaType.APPLICATION_JSON_VALUE, consumes =
    // MediaType.APPLICATION_JSON_VALUE
    ...
    public @ResponseBody String getStudent(@PathVariable int sId, @ModelAttribute("formBean") FormBean formBean,
            Model model) {
        ...
        Query query = session.createQuery(" from Student where id=" + x); // select
                                                                          // name,
                                                                          // id,
                                                                          // age
                                                                          // from
        ...
        /*
         * for(Student s:studentList){ System.out.println(s.getName()); }
         */
        // System.out.println(student);
        // model.addAttribute("student", student);// when Model type is used for
        // model
        // model.put("studentList", studentList); // when Map type is used for
        // model

        /*
         * Query query=session.createQuery("FROM Student"); List<Student>
         * studentList=query.list(); System.out.println(studentList);
         * model.addAttribute("studentList", studentList);
         */

        /*
         * Student student=new Student(); student.setAge(50); student.setId(3);
         * student.setName("Krishna"); session.saveOrUpdate(student);
         */
    ...
    @RequestMapping(value = "/{dName}/{id}", method = RequestMethod.GET)
    // dName is department name
    public String getStudentOrTeacher(@PathVariable String dName, @PathVariable int id,
        ...
        String dept = null;
        int idNo = id;
        if (dName.startsWith("s"))
            dept = "Student";
        if (dName.startsWith("t"))
            dept = "Teacher";

        Query query = session.createQuery(" from" + "  " + dept + " where id=" + idNo); // select
                                                                                        // name,
                                                                                        // id,
                                                                                        // age
                                                                                        // from
         ...
         * List<Student> studentList=query.list();
         * 
         * Student s=studentList.get(0); model.addAttribute("student",s);
         * model.addAttribute("studentList", studentList);
         */
    ...
    public @ResponseBody String callAjax(@ModelAttribute("formBean") FormBean formBean, Model model) {
        ...
        Query query = session.createQuery(" from Student where id=199"); // select
                                                                         // name,
                                                                         // id,
                                                                         // age
                                                                         // from
